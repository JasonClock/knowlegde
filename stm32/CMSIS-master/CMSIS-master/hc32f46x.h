...
//定义用于连锁传输的DMA通道描述符(descriptor)， 8 个寄存器共 8 个 word
//起始于M4_DMA_TypeDef中的SAR0，SAR1，SAR2，SAR3
typedef struct{
  //源地址
  uint32_t SAR;  
  //目标地址
  uint32_t DAR;   
  //传输次数(b16-31)，与数据块大小(b0-9)
  uint32_t DTCTL; 
  //目标地址重复区域大小(b16-25)，与源地址重复区域大小(b0-9),SRPTEN/DRPREN时有效
  uint32_t RPT;  //与 DMA_PRTBx 共用
  //源设备不连续地址传输控制，源地址跳转的数据量(b31-20)，与源地址跳转的地址偏移量(b0-19)
  uint32_t SNSEQCTL; //与 DMA_SNSEQCTLBx 共用
  //目标设备不连续地址传输控制，目标地址跳转的数据量(b31-20)，与目标地址跳转的地址偏移量(b0-19)
  uint32_t DNSEQCTL; //与 DMA_DNSEQCTLBx 共用
  //链指针寄存器,
  uint32_t LLP;
  //通道控制:b12中断使能,b11连锁传输模式选择(0中断传输载入后立即传输)
  //        b10: 连锁传输使能, b9-b8：传输宽度(0:8b,1:16b,2:32b)，b7：目标地址不连续传输使能
  //        b6：源地址不连续传输使能,b5：目标重复传输功能使能,b4源重复传输功能使能
  //        b2-b3：目标地址的更新方式(0固定1递增2递减),b0-b1：源地址的更新方式(0固定1递增2递减),
  uint32_t CHCTL;
}M4_DMA_CH_TypeDef;
